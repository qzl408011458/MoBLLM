...
def get_chat_completion(prompt):
    ...
    # if '}' in res_content:
    #     res_content = res_content[:res_content.find('}') + 1]
...
def load_lora_model(model_path, lora_path):
    ...
    # lora_path = '/root/autodl-tmp/output/llama3_1_instruct_lora0/checkpoint-100'  # 这里改称你的 lora 输出对应 checkpoint 地址
...
def parse_args(argv=None):
    ...
    parser.add_argument('--ft_path', default='', type=str,
...
def main():
    ...
    if args.ft_name == 'loftq':
        ...
        if len(args.ft_path) != 0 and args.ft_path != model_path:
            # model = load_lora_model(model_path, lora_path)
            # 加载lora权重
            model = PeftModel.from_pretrained(model, model_id=lora_path)
            output_dir = f"output/ft_{ft_name}_{dataname}/top{top_k}"  # the output path
            log_dir = f"logs/ft_{ft_name}_{dataname}/top{top_k}"  # the log dir
            ...
            output_dir = f"output/{ft_name}_{dataname}/top{top_k}"  # the output path
            log_dir = f"logs/{ft_name}_{dataname}/top{top_k}"  # the log dir
