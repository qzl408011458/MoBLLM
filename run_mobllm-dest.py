...
def parse_args(argv=None):
    ...
    parser.add_argument('--data', default='dest_hk_incident', type=str,
    ...
    parser.add_argument('--use_gpt', default=False, type=bool,
                        help='if use GPT')
...
def main():
    ...
    # args.use_gpt = False
    if args.use_gpt:
        output_dir = f"output_api/llmmob_{model_gpt}_{dataname}/top{top_k}"  # the output path
        ...
        if args.ft_name == 'loftq':
            ...
            if len(args.ft_path) != 0 and args.ft_path != model_path:
                ...
                model = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=torch.bfloat16, use_cache=False,
                                                             device_map=args.device)
                output_dir = f"output/raw_{dataname}/top{top_k}"  # the output path
                log_dir = f"logs/raw_{dataname}/top{top_k}"  # the log dir
