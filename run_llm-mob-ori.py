...
def single_query_top1(X):
    ...
    Stays in both <history> and <context> are in chronological order.
...
def single_user_query(uid, predict_X, predict_y,logger, top_k, is_wt, output_dir, sleep_query, sleep_crash):
    ...
    #for query in queries[processed_queries:]:
        try:
            ...
            logger.info(f"API request failed for the {i+1}th query")
            time.sleep(sleep_crash)
            break  # Exit the loop if API crashes
    ...
    if len(current_results) < total_queries:
        ...
        single_user_query(uid, predict_X, predict_y, logger, top_k, is_wt, output_dir, sleep_query, sleep_crash)
...
def main():
    global test_uids_count, model_gpt, test_data
    # model_gpt = 'GPT35_LingoTrip'
    parse_args()
    ...
    if args.data == 'ori_hk':
        with open(f'data4eval/ori_hk.pkl', 'rb') as fr:
