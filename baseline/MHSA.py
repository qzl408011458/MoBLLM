...
class MHSA(nn.Module):
    def __init__(self, st_sum, model_dim, nhead=8, layers=1, task='ori',
        ...
        # emb_all_dims = 20 + 20 + 8 + 8 + 4

        self.__init_EmbedModules()
        # self.lin2enc = nn.Linear(self.model_dim, model_dim, bias=False)
    ...
    def __init_EmbedModules(self):
        self.emb_sloc = nn.Embedding(self.st_sum + 2, self.model_dim)
        self.emb_eloc = nn.Embedding(self.st_sum + 2, self.model_dim)
        self.ph_eloc = nn.Parameter(torch.randn(self.model_dim))

        self.emb_w = nn.Embedding(7 + 1, self.model_dim)

        self.emb_smin = nn.Sequential(
            nn.Linear(1, 32, bias=False),
            nn.Tanh(),
            nn.Linear(32, self.model_dim, bias=False),
        )
        ...
        if self.task == 'ori':
            self.ph_dura = nn.Parameter(torch.randn(self.model_dim))
    ...
    def forward(self, x, y_tar):
        ...
        input_enc1 = out_emb_sloc + out_emb_eloc + out_emb_smin + out_emb_dura + out_emb_w

        # input_enc2 = torch.cat([out_emb_sloc_tar, out_emb_eloc_tar, out_emb_smin_tar,
        #                        out_emb_dura_tar, out_emb_w_tar], dim=-1)
        input_enc2 = out_emb_sloc_tar + out_emb_eloc_tar + out_emb_smin_tar + out_emb_dura_tar + out_emb_w_tar

        input_enc = torch.cat([input_enc1, input_enc2], dim=1)

        input_enc = input_enc.transpose(0, 1)
